...
from djsonapi_client import Collection
...
class TestCollectionFetch:
    async def test_fetch_populates_data(self, article_type, article_list_response):
    ...
    async def test_fetch_sets_links(self, article_type, article_list_response):
        sdk = article_type._sdk
        col = Collection(sdk, f"{HOST}/articles")
        async with sdk:
            with mock_get(sdk, payload=article_list_response):
    ...
    async def test_fetch_sets_meta(self, article_type, article_list_response):
        sdk = article_type._sdk
        col = Collection(sdk, f"{HOST}/articles")
        async with sdk:
            with mock_get(sdk, payload=article_list_response):
    ...
    async def test_fetch_idempotent(self, article_type, article_list_response):
        sdk = article_type._sdk
        col = Collection(sdk, f"{HOST}/articles")
        async with sdk:
            with mock_get(sdk, payload=article_list_response):
    ...
    async def test_fetch_force_refetch(self, article_type, article_list_response):
        sdk = article_type._sdk
        col = Collection(sdk, f"{HOST}/articles")
        async with sdk:
            with mock_get(sdk, payload=article_list_response):
...
class TestCollectionChaining:
    def test_filter(self, sdk):
        col = Collection(sdk, f"{HOST}/articles").filter(title="hello")
        assert col._params["title"] == "hello"
    ...
    def test_filter_chains(self, sdk):
        ...
        assert col._params == {"title": "hello", "author": "42"}
...
class TestCollectionPagination:
    ...
    async def test_all_pages_follows_next(self, article_type):
        ...
        col = Collection(sdk, f"{HOST}/articles")
        async with sdk:
            with mock_get(sdk, payload=p1):
                await col.fetch()
                with mock_get(sdk, payload=p2):
                    pages = []
                    async for p in col.all_pages():
                        await p.fetch()
                        pages.append(p)
                    assert len(pages) == 2
